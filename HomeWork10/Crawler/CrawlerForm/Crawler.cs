  ...
  class Crawler {
    ...
    public void Start() {
      ...
      foreach(string s in pending)
            {
                pending.TryDequeue(out string _);
            }
      pending.Enqueue(StartURL);
       //     int count = 0;
      //while (count < MaxPage && pending.Count > 0) {
        bool _ = pending.TryDequeue(out string url);
       try {
   ...
   private void DownLoadingAndParsing(string url)
        {
            int count = 0;
            while (count < MaxPage && pending.Count > 0)
            {
                bool _ = pending.TryDequeue(out string _);
                try
                {
                    string html="";
                    lock (this)
                    {
                        html = DownLoad(url); // 下载
                    }
                    urls[url] = true;
                    PageDownloaded(this, url, "success");
                    Parse(html, url);//解析,并加入新的链接
      ...
      foreach (Match match in matches) {
        ...
        if (Regex.IsMatch(host, HostFilter) && Regex.IsMatch(file, FileFilter)) {
          if (!urls.ContainsKey(linkUrl)) {
            ...
            Task task = Task.Run(() => DownLoadingAndParsing(linkUrl));
          }
        }
      }
    }
